...
from utils.evaluation import get_latest_model, update_leaderboard_roc
from utils.data_processing import preprocess_real_data
...
def evaluate_real_world(model):
    ...
    for name, gps in REAL_EVENTS.items():
        try:
            ...
            data = preprocess_real_data(strain, FS)
    ...
    try:
        long_strain = TimeSeries.fetch_open_data('H1', REAL_NOISE_START, 
                                                 ...
                                                 verbose=False)
        clean_data = preprocess_real_data(long_strain, FS)
...
def main():
    ...
    update_leaderboard_roc(os.path.basename(model_path), sim_res, real_res, LEADERBOARD_FILE, BASELINE_FILE)
